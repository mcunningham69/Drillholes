...
{
    public class IntervalValidation : IIntervalValidation
    {
        private ValidationIntervalDto intervalValidationDto;
        public IntervalValidation()
        {
            intervalValidationDto = new ValidationIntervalDto();

            intervalValidationDto.isValid = true;
        }
        public async Task<ValidationIntervalDto> CheckForDuplicates(ValidationMessages ValuesToCheck, XElement assayValues)
        {
                ...
                foreach (var hole in holes)
                {
                    ...
                    if (duplicates.Count > 0)
                    {
                        foreach (string distance in duplicates)
                        {
                            ...
                            check.ValidationStatus.Add(new DrillholeValidationStatus { ErrorType = DrillholeMessageStatus.Warning, Description = message, ErrorColour = "Orange", holeID=hole });
        ...
        public async Task<ValidationIntervalDto> CheckForMissingCollars(ValidationMessages ValuesToCheck, List<XElement> drillholeValues)
        {
            intervalValidationDto.testMessages = ValuesToCheck;
            string message = "";
            foreach (var check in ValuesToCheck.testMessage)
            {
                if (check.validationTest == DrillholeConstants.checkHole)
                {
                    ...
                    message = "Collar and Interval table have same ordinal hole IDs";
                    ...
                    else
                    {
                        ...
                        if (result)
                        {
                            message = "Collar and Interval table have same hole IDs";
                        ...
                        else
                        {
                            if (collarHoles.Except(distanceHoles).Count() > 0)
                            {
                                ...
                                {
                                    message = "Hole '" + dist + "' has no corresponding hole in the Interval table";
                            ...
                            if (distanceHoles.Except(collarHoles).Count() > 0)
                            {
                                ...
                                {
                                    message = "Interval hole '" + dist + "' has no corresponding value in the Collar table";
                                    check.ValidationStatus.Add(new DrillholeValidationStatus { ErrorType = DrillholeMessageStatus.Warning, Description = message, ErrorColour = "Orange" });
                                    check.validationMessages.Add(message);
                                }
                            }
        ...
        public async Task<ValidationIntervalDto> CheckForNumeric(ValidationMessages ValuesToCheck, XElement assayValues)
        {
            intervalValidationDto.testMessages = ValuesToCheck;
            ...
            foreach (var check in ValuesToCheck.testMessage)
            {
                ...
                else
                {
                    ...
                    CheckNumericValues(assayValues, check, fieldID, fieldName, holeName);
                }
        ...
        public async Task<ValidationIntervalDto> CheckIsEmpty(ValidationMessages ValuesToCheck, XElement assayValues)
        {
            intervalValidationDto.testMessages = ValuesToCheck;
            ...
            foreach (var check in ValuesToCheck.testMessage)
            {
                ...
                CheckEmptyValues(assayValues, check, fieldID, fieldName, holeName);
            }

            return intervalValidationDto;
        }

        private async void CheckEmptyValues(XElement assayValues, ValidationMessage validationTest, string fieldID, string fieldName, string holeName)
        {
            var elements = assayValues.Elements();
            ...
            foreach (XElement element in elements)
            {
                ...
                if (string.IsNullOrEmpty(valueCheck) || string.IsNullOrWhiteSpace(valueCheck))
                {
                    ...
                    message = "Interval record " + holeAttr + " and field '" + fieldID + "' of field type '"
                        + fieldName + "' has no value";

                    validationTest.validationMessages.Add(message);
                    validationTest.ValidationStatus.Add(new DrillholeValidationStatus { ErrorType = _errorType, Description = message, ErrorColour = _errorColour, holeID=hole, id=Convert.ToInt32(holeAttr) });
                ...
                else
                {
                    ...
                    validationTest.validationMessages.Add(message);
                    validationTest.ValidationStatus.Add(new DrillholeValidationStatus { ErrorType = DrillholeMessageStatus.Valid, Description = message, ErrorColour = "Green", holeID = hole, id = Convert.ToInt32(holeAttr) });
        ...
        private async void CheckNumericValues(XElement drillholeValues, ValidationMessage validationTest, string fieldID, string fieldName, string holeName)
        {
            ...
            foreach (XElement element in elements)
            {
                ...
                if (!Information.IsNumeric(fieldValue))
                {
                    ...
                    message = "Interval value for record " + holeAttr + " and field '" + fieldID + "' of field type '"
        ...
        public async Task<ValidationIntervalDto> CheckMaxDepth(ValidationMessages ValuesToCheck, List<XElement> drillholeValues)
        {
            intervalValidationDto.testMessages = ValuesToCheck;
            ...
            foreach (var check in ValuesToCheck.testMessage)
            {
                ...
                if (check.validationTest == DrillholeConstants.checkTD)
                {
                    foreach (XElement element in collarElements)
                    {
                        ...
                        var holeAttr = element.Attribute("ID").Value;
                        ...
                        if (Information.IsNumeric(length))
                        {
                            ...
                            if (assayIntervals.Count > 0)
                            {
                                ...
                                if (assayMaxDepth > tD)
                                {
                                    message = "Interval value has a maximum distance of " + assayMaxDepth.ToString() +
                                        " which is greater than  length " + tD + " for collar " + collar;

                                    check.validationMessages.Add(message);
                                    check.ValidationStatus.Add(new DrillholeValidationStatus { ErrorType = DrillholeMessageStatus.Warning, Description = message, ErrorColour = "Orange", holeID=collar, id=Convert.ToInt32(holeAttr)});
                                    check.verified = false;
                                }
                                else
                                {
                                    ...
                                    message = "Interval max. distance of " + assayMaxDepth + " for hole " + collar + " is " + submessge + " the total length of " + tD;
                                    check.ValidationStatus.Add(new DrillholeValidationStatus { ErrorType = DrillholeMessageStatus.Valid, Description = message, ErrorColour = "Green" });
                                    check.validationMessages.Add(message);
                                }
                            }
        ...
        public async Task<ValidationIntervalDto> CheckMissingIntervals(ValidationMessages ValuesToCheck, List<XElement> drillholeValues)
        {
            intervalValidationDto.testMessages = ValuesToCheck;
                            ...
                            for (int v = 0; v < dblFrom.Count; v++)
                            {
                                ...
                                if (v == 0)
                                {
                                    if (dblF > 0)
                                    {
                                        ...
                                        check.validationMessages.Add(message);
                                        check.ValidationStatus.Add(new DrillholeValidationStatus { ErrorType = DrillholeMessageStatus.Warning, Description = message, ErrorColour = "Orange", holeID=hole, id = Convert.ToInt32(holeAttr[v]) });
                                        check.verified = false;
                                    }
                                }
        ...
        public async Task<ValidationIntervalDto> CheckNegativeIntervals(ValidationMessages ValuesToCheck, XElement drillholeValues)
        {
            intervalValidationDto.testMessages = ValuesToCheck;
                                ...
                                if (Information.IsNumeric(fromVals[d]))
                                {
                                    ...
                                    if (Information.IsNumeric(toVals[d]))
                                    {
                                        ...
                                        if (dblTo == 0)
                                        {
                                            ...
                                            check.validationMessages.Add(message);
                                            check.ValidationStatus.Add(new DrillholeValidationStatus { ErrorType = DrillholeMessageStatus.Error, Description = message, ErrorColour = "Red",
                                                holeID = hole, id = Convert.ToInt32(holeAttrs[d])});
                                        ...
                                        else if (dblTo < 0)
                                        {
                                            ...
                                            check.ValidationStatus.Add(new DrillholeValidationStatus { ErrorType = DrillholeMessageStatus.Warning, Description = message, ErrorColour = "Orange", holeID = hole, id = Convert.ToInt32(holeAttrs[d]) });
                                            check.validationMessages.Add(message);
                                            check.verified = false;
                                        }
        ...
        public async Task<ValidationIntervalDto> CheckOverlappingIntervals(ValidationMessages ValuesToCheck, XElement drillholeValues)
        {
            intervalValidationDto.testMessages = ValuesToCheck;
        ...
        public async Task<ValidationIntervalDto> CheckStructuralMeasurements(ValidationMessages ValuesToCheck, XElement continuousValues)
        {
            intervalValidationDto.testMessages = ValuesToCheck;
